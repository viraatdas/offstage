...
## 2026-08-21: the open question below was answered

This document ends by asking which of two things to pay for: an SRP
implementation to reach Screen Sharing's virtual display, or nothing at all if a
background session turns out to be drivable as-is. That was settled by
measurement, and the answer is the cheap one.

**A non-console session is fully drivable.** Screenshot, click, keyboard, drag
and scroll all work inside a background Aqua session, and a headed Chromium ran
a Playwright spec to completion there while the console user kept working. No
virtual display and no protocol work was needed.

**The SRP route was probed and is not worth building.** Against
`screensharingd` on 127.0.0.1:5900, security types 33, 35 and 36 all wait for
the client to speak first. Sweeping the sub-type byte 0..255 under type 33, and
0..63 under 35 and 36, exactly one value ever answers: 30, which is the legacy
ARD Diffie-Hellman challenge documented below, the path that takes over the
console. No SRP handshake byte was ever obtained, and the `bad packet:
version:%d, authtype:%s` parser sits behind a gate that probing did not reach.
Getting further means disassembling `HandleSRPAuthenticationMessage` and
