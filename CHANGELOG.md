...
- **`native/sessiond/smoke.sh` passes again.** Its `apps` check asserted an
  exact key set that was never updated when the daemon started reporting each
  app's activation `policy` in 0.3.4, so the command AGENTS.md and the README
  both promise prints `SMOKE PASSED` had been failing since that release.

### Removed

- **The RFB / Screen Sharing client is gone** (`src/screen/rfb.ts` and its
  test, 585 lines). Nothing imported it. It could only complete the legacy ARD
  auth type, and completing that auth type moves the console to the
  authenticating user, which is the one thing the session lane exists to
  prevent. `docs/macos-sessions.md` keeps the analysis that produced it.
- `sessionSharePlan`, an exported helper with no callers and no tests.

### Changed

- **No source file is over 1,000 lines any more.** `src/router/signals.ts`
  (2,210) and `src/cli/api.ts` (2,045) are split along the seams already drawn
  in them, into modules that form a graph with no cycles. The router gains
  `signal.ts`, `views.ts`, `flags.ts`, `bins.ts`, `macos.ts`, `tools.ts`,
  `webdriver.ts` and `configs.ts`; the CLI gains `session.ts` (bring the lane
  up) and `session-control.ts` (drive one that is up). The published package
  API, the CLI, and the MCP tools are unchanged.
- **Nothing describes offstage in terms of a virtual machine.** The `vm` lane
  was deleted in 0.3.0, but the contract header still said "all four lanes"
  and "inside a VM", the headless lane still told users "no virtual machine
  was booted", and the entitlements probe framed its whole verdict around a
  disposable Tart VM. The probe now answers what it actually measures: can
  ad-hoc signing carry this app's entitlements, or does it need a real
  Developer ID? AGENTS.md carries the rule so it does not drift back.
- Em dashes are gone from the repository, replaced with the punctuation each
  one meant. `explain()` had been using one as a field separator, where a
  colon would collide with the reason text, so it now joins with ` | `.
